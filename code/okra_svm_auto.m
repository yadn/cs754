...
outDirectory = sprintf('./result/matnew_auto');
mkdir(outDirectory);
...
%% show mask
% figure;imshow(mask);
name = sprintf('%s/adaptive_mask1st_%d_template.mat',outDirectory,validImNo);
% save(name,'mask');
...
%% ssim calculation
test = mat2gray(testIm);figure;imshow(test);
testpilot = mat2gray(pilot_test);figure;imshow(testpilot);
test_wted_recon = mat2gray(resultWeightedPrior1);figure;imshow(test_wted_recon);
test_irrad = mat2gray(reconIrradiation);figure;imshow(test_irrad);
...
% fh = figure;imshow(residual_test_weiner,[]);
